...
public class CafeCommon1Activity extends BasicActivity {
    ...
    @BindView(R.id.cafe_address_btn)
    ImageView mapIcon;

    @BindView(R.id.cafe_title_edit)
    EditText nameEdit;

    @BindView(R.id.cafe_address_edit)
    EditText areaEdit;

    @BindView(R.id.cafe_phone)
    EditText phoneEdit;

    @BindView(R.id.cafe_web)
    EditText webEdit;

    @BindView(R.id.cafe_time)
    EditText timeEdit;

    @BindView(R.id.cafe_plus)
    EditText plusEdit;

    @BindView(R.id.cafe_caution)
    EditText cautionEdit;

    private String area = "";                                            // area = splitArea()함수를 통해서 받아오는 값,
    private String name, wholeArea, phone, web, time, plus, caution;    // wholeArea = 사용자 입력 주소
