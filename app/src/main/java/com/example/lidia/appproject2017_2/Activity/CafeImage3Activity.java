...
public class CafeImage3Activity extends BasicActivity {
    ...
    @BindView(R.id.cafe3_image)
    ImageView recentImage;

    @BindView(R.id.cafe3_recycler)
    RecyclerView imageRecycler;

    @BindView(R.id.cafe3_camera)
    ImageView camera;
        ...
        public void onClick(View view) {
            switch (view.getId()){
                ...
                case R.id.cafe3_done:
                    ...
                    if(areaSection!=null){
                        DatabaseReference storeRef = mDatabase.child("Cafe").child(areaSection).push();
                        String storeKey = storeRef.getKey();
                        cafeModel.storeImage(changeToInputStream(mImageUriList), storeRef, storeKey, finalBundle);
                        showAlertDialog("포스트를 등록하였습니다", "확인");
                    }


                    break;
                case R.id.cafe3_camera:
    ...
    private void showAlertDialog(String mainText, String buttonText){
        AlertDialog dialog = new AlertDialog.Builder(CafeImage3Activity.this)
                .setMessage(mainText)
