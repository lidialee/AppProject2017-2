...
public class EtcImage3Activity extends BasicActivity {
    ...
    @BindView(R.id.etc3_image)
    ImageView recentImage;

    @BindView(R.id.etc3_recycler)
    RecyclerView imageRecycler;

    @BindView(R.id.etc3_camera)
    ImageView camera;
        ...
        public void onClick(View view) {
            switch (view.getId()){
                ...
                case R.id.etc3_done:
                    ...
                    if(areaSection!=null){
                        DatabaseReference storeRef = mDatabase.child("Etc").child(areaSection).push();
                        String storeKey = storeRef.getKey();
                        etcModel.storeImage(changeToInputStream(mImageUriList), storeRef, storeKey, finalBundle);
                        showAlertDialog("포스트를 등록하였습니다", "메인으로 돌아가기");
                    }
                    break;
                case R.id.etc3_camera:
    ...
    private void showAlertDialog(String mainText, String buttonText){
        AlertDialog dialog = new AlertDialog.Builder(EtcImage3Activity.this)
                .setMessage(mainText)
